...
sub load {
    my (
        $web,         $topic,    $text,        $purpose, $expandText,
        $withDeleted, $setClass, $objectClass, $reltopic
    ) = @_;
    $withDeleted ||= 0;
    ...
    unless ( defined $text ) {
        ...
        Foswiki::Plugins::ObjectPlugin::checkAccess( $purpose, $web, $topic,
            $text, $meta );    # can throw AccessControlException
        $objectSet->{meta} = $meta;
    }
    ...
    while ( $i < scalar(@blocks) ) {
        ...
        if ( $block =~ /^%OBJECT{(.*)}%$/ ) {
            ...
            if ( $add && $reltopic ) {
                $add = 0
                  unless ( $object->{reltopic}
                    && $object->{reltopic} eq $reltopic );
            }
            ...
            if ($block) {
                my $newobjects = load(
                    $web,      $topic,       $block,
                    $purpose,  0,            $withDeleted,
                    $setClass, $objectClass, $reltopic
                );
...
sub loadToFind {
    ...
    unless ( defined $limit ) {
        $limit =
          31;    # SMELL: magic number, no theory behind it, make configurable
    }
    elsif ($limit) {
...
# even limit not allowed because of the OBJECT|ENDOBJECT pair and the use of split()
    ...
    while ($text) {
        ( $attrs, $descr, $pre, $post, $text ) =
          parseForObject( $text, $pre, $limit, $uid, $withDeleted );
    }
    my $found =
      $attrs ? $objectClass->new( $web, $topic, $attrs, $descr, $meta ) : undef;
...
sub parseForObject {
    ...
    if ( $limit && $blocks[-1] =~ /%OBJECT{.*?}%/ )
    {    # more than $limit number of objects
        $extra = pop(@blocks)
          ;    # put the extra objects on side to be parsed later if need be
    }
    while ( scalar(@blocks) ) {
    ...
    foreach my $topic ( keys %$grep ) {
        ...
        next
          unless Foswiki::Func::checkAccessPermission( 'VIEW',
            ...
            $text, $topic, $web, $meta );
        my $tobjs =
          Foswiki::Plugins::ObjectPlugin::ObjectSet::load( $web, $topic, $text,
            undef, 0 );
