...
sub new {
    ...
    my $this =
      bless( {}, $class );    # $class->SUPER::new($session, $web, $topic);
...
# throw Foswiki::OopsException('object',web=>$web,topic=>$topic,def=>'badobject',params=>[$attrs]) unless $type;
    ...
    foreach my $key ( keys %$attr ) {
        ...
        $fieldType ||= 'text'
          ; # dynamically generated objects can contain whatever attributes they want
...
"ObjectPlugin - an object definition for type $type cannot be found. Using default params only."
    ...
    if ($formWeb) {
        ...
        foreach my $fieldDef ( @{ $objectDef->{fields} } ) {
            ...
            if ( $fieldDef->{type} =~ /noload/ )
            {    # cache the values for css, js and sanitise
    ...
    while ( -f $lockFile ) {
        ...
        if ( time() - $s[9] > 10 * 60 ) {
            Foswiki::Func::writeWarning(
                "Object Plugin: Warning: broke $lockFile");
            last;
        }
        sleep(1);
    }
...
sub stringify {
    ...
    foreach my $key ( sort keys %$this ) {
        ...
        if ( $key eq 'text' ) {
            $descr = $this->{text};
            $descr =~
              s/^(?:\t|   )+(\*\s+?(?:Set|Local)\s+?\w+\s*?=\s*?.*?)$/>>>$1/mg
              ;    # no sneaking in any preferences
                   # $descr =~ s/^\s{3}/\t/os;
            $descr =~ s/\s+$//os;
...
sub formatTime {
    ...
    if ( !$time ) {
        ...
        $stime = Foswiki::Func::formatTime( $time,
            '$wday, $day $month $year - $hours:$minutes', 'servertime' );
    }
    return $stime;
}
...
sub _matchType_names {
    ...
    foreach my $name ( split( /\s*,\s*/, $val ) ) {
        ...
        eval {
            $r =
              ( $this->{$vbl} =~ m/$name,\s*/ || $this->{$vbl} =~ m/$name$/ );
        };
        return 1 if ($r);
    }
    return 0;
...
sub createFromQuery {
    ...
    return new Foswiki::Plugins::ObjectPlugin::Object( $web, $topic, $attrs,
        $desc );
}
...
sub updateFromQuery {
    ...
    if ( $this->{edited} ) {
        ...
        return
          if ( $now - Foswiki::Time::parseTime($lastEdit) ) <
...
sub deleteObject {
    ...
    $this->{deleted} =
      formatTime( $now, 'attr' ) . '=' . Foswiki::Func::getWikiName();
}
...
sub _getTemplate {
    ...
    Foswiki::Plugins::ObjectPlugin::writeDebug(
        'tmpls->{type.tail} must be empty, getting template from file');
    ...
    if ( !$templates ) {
        my $templateFile =
          Foswiki::Func::getPreferencesValue('OBJECTPLUGIN_DEFAULTTEMPLATES')
          || 'Object';
        Foswiki::Plugins::ObjectPlugin::writeDebug(
...
# done like this so _getTemplate can load the js and css so it can be used in the format
    ...
    foreach my $field ( keys %$fields ) {
        ...
        if ( $type =~ m/multi/o ) {
            $val =
              defined $this->{$field} ? join( ',', @{ $this->{$field} } ) : '';
        }
        elsif ( $type =~ m/date/o ) {
            $val =
              formatTime( Foswiki::Time::parseTime( $this->{$field} ),
                'string' );
        }
    ...
    $descr =~ s/%(\w*OBJECT\w*)\{/%<nop>$1\{/gso;
    $descr =~
      s/<(\/?script)/&lt;$1/gsoi;    # knobble any attempt at running scripts
    ...
    if ( $tmpl =~ /%ATTACHMENTS%/ ) {
        ...
        if ( $this->{attachments} ) {
            ...
            foreach my $attachment ( @{ $this->{attachments} } ) {
                ...
                my $fileType =
                  $session->mapToIconFileName($attachment); # SMELL: no func api
                ...
                my $icon =
                  ...
                  . '" border="0" />';
                $atchs .=
"   * $icon [[%PUBURL%/$this->{web}/$this->{topic}/$attachment][$attachment]]\n";
            }
        }
...
sub renderForEdit {
    ...
    $tmpl =~ s/%UID%/$this->{uid}/g
      ; # here because of ADDTOZONE which otherwise would remove the tag as part
    ...
    if ( $formDef->{fields} )
    {    # only if the object has an object definition topic
        my $meta =
