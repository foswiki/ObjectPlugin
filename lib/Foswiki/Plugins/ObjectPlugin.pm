...
use vars
  qw( $VERSION $RELEASE $SHORTDESCRIPTION $disableStandardProcessing $debug $defaultsAdded );
$debug            = 0;                                   # toggle me
$VERSION          = '$Rev$';
$RELEASE          = '1.01';
...
'Version mismatch between ObjectPlugin and Plugins.pm $Foswiki::Plugins::VERSION. 1.026 required.'
    ...
    $Foswiki::Plugins::SESSION->{objectplugin} =
      { forms => {}, templates => {} };
    ...
    Foswiki::Func::registerTagHandler( 'OBJECTSEARCH', \&_handleObjectSearch,
        'context-free' );
    Foswiki::Func::registerTagHandler( 'OBJECTBUTTON', \&_handleObjectButton,
        'context-free' );
...
sub commonTagsHandler {
    ...
    my $os =
      Foswiki::Plugins::ObjectPlugin::ObjectSet::load( $_[2], $_[1], $_[0],
        undef, 0, 1 );
...
sub _handleObjectSearch {
    ...
    my $objects =
      Foswiki::Plugins::ObjectPlugin::ObjectSet::allObjectsInWebs( $web, $attrs,
        0 );
    $objects->sort($sort);
...
HERE
    addDefaultsToPage();
    Foswiki::Func::addToZone( 'script', 'OBJECTBUTTON_JS',
        <<"HERE", 'OBJECTPLUGIN_JS' );
...
HERE
    ...
    if ( scalar(@types) > 1 ) {
        ...
        $select =
          '<input type="hidden" value="' . $types[0] . '" id="objectType">';
    }
...
sub addDefaultsToPage {
    ...
    my $src =
      $debug ? '_src' : '';    # not using this at the moment, object$src.js

    Foswiki::Func::addToZone( 'head', 'OBJECTPLUGIN_CSS',
        <<HERE, 'JQUERYPLUGIN::THEME' );
<link rel="stylesheet" href="%PUBURLPATH%/%SYSTEMWEB%/ObjectPlugin/object.css" type="text/css" media="all" />
HERE
    Foswiki::Func::addToZone( 'script', 'OBJECTPLUGIN_JS',
        <<"HERE", 'HIJAXPLUGIN_JS' );
...
sub _objectUpdate {
    ...
    my ( $object, $pre, $post, $meta ) =
      Foswiki::Plugins::ObjectPlugin::ObjectSet::loadToFind( $uid, $web, $topic,
        undef, 'CHANGE' );
...
sub _objectNew {
    ...
    checkAccess( 'CHANGE', $web, $topic, $text, $meta )
      ;    # can throw AccessControlException

    my $object =
      Foswiki::Plugins::ObjectPlugin::Object::createFromQuery( $web, $topic,
        $query );
...
sub _objectView {
    ...
    if ( $context eq 'edit' && !$uid ) {
        ...
        checkAccess( 'CHANGE', $web, $topic, $text, $meta )
          ;    # can throw AccessControlException
        return new Foswiki::Plugins::ObjectPlugin::Object( $web, $topic,
            'type="' . $type . '"',
            '', $meta );
    }
...
sub _objectMove {
    ...
    my ( $newweb, $newtopic ) =
      Foswiki::Func::normalizeWebTopicName( '', $target );
    ...
    my $woa = $query->param('woattachments')
      || 0;    # default is to move with associated attachments
    ...
    if ( $newweb eq $Foswiki::cfg{TrashWebName} ) {    # trashing an object
        ...
        ( $newmeta, $newtext ) = Foswiki::Func::readTopic( $newweb, $newtopic );
        checkAccess( 'CHANGE', $newweb, $newtopic, $newtext, $newmeta )
          ;    # can throw AccessControlException
    }
        ...
        unless ($woa) {    # move with attachements
            foreach my $attachment ( @{ $object->{attachments} } ) {
                ...
                Foswiki::Func::moveAttachment( $web, $topic, $attachment,
                    $newweb, $newtopic, undef )
                  if Foswiki::Func::attachmentExists( $web, $topic,
                    $attachment );
            }

            # meta data has changed, so...
            ( $newmeta, $newtext ) =
              Foswiki::Func::readTopic( $newweb, $newtopic );
    ...
    $disableStandardProcessing = 1;
    Foswiki::Func::saveTopic( $newweb, $newtopic, $newmeta, $newtext,
        { comment => "moved object $uid to here from $web.$topic" } );

    $Foswiki::Plugins::SESSION->{user} =
      $curUser;    # switch back incase we did a switch to admin

    Foswiki::Func::saveTopic(
        $web, $topic, $meta,
        $pre . $post,
        { comment => "(re)moved object $uid to $newweb.$newtopic" }
    );
    return $object;
}
...
sub insertObject {
    ...
    if ( $target eq 'section' ) {
        ...
        my ( $obj, $pre, $post, $meta ) =
          Foswiki::Plugins::ObjectPlugin::ObjectSet::loadToFind( $match,
            $object->{web}, $object->{topic}, $text, undef, 1 );
...
sub formatResponse {
    ...
    if ( $query->param('asobject') ) {
        ...
        # writeDebug(Dumper($object));
        my $json =
          JSON->new->allow_nonref->allow_blessed->convert_blessed->encode(
            $object);
    ...
    my $result =
        $context eq 'edit'
      ? $object->renderForEdit()
      : $object->renderForDisplay();
...
sub checkAccess {
    ...
    my $access =
      Foswiki::Func::checkAccessPermission( $type, $user, $text, $topic, $web,
        $meta );
    throw Foswiki::AccessControlException( $type, $user, $web, $topic,
        $Foswiki::Plugins::SESSION->security->getReason() )
      unless $access;
}
